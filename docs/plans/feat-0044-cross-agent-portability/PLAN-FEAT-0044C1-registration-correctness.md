...
Status: in progress; Milestones 1 and 2 complete, Milestone 3 not started. Author: Joe-Degler (verified by `gh api user` in the parent session). Date: 2026-09-07.

This ExecPlan is a living document maintained under `docs/plans/PLANS.md`. Planning is complete. Milestones 1 and 2 passed independent code review, QA and validation. The next implementation milestone is Milestone 3.
...
- [ ] Milestone 3: effective Codex home and recoverable registration receipts; unit, Docker E2E, review, QA, and domain updates.
...
All paths below are relative to `/opt/development/clooks`. `CLAUDE.md` supplies repository workflow and Docker requirements. `docs/plans/PLANS.md` governs this living document. Prior completed Plan C is `docs/plans/feat-0044-cross-agent-portability/PLAN-FEAT-0044C-codex-registration-entrypoint.md`; its historical results stand, but C1 replaces the destructive shape and substring-detection decisions. The source-confirmed follow-up rationale is in `continuation-review.md` in the same directory. Parent reports `ATTENTION.md` now includes planned C1 dangers; parent retains its ownership and checks alignment before implementation.
...
Milestone 3 unit command (give the existing engine suite fresh per-test state roots before running it, as specified in Related Findings):
    ...
    bun test src/registration-state.test.ts src/commands/init.test.ts src/commands/uninstall.test.ts src/agents/codex/settings.test.ts src/settings.test.ts src/engine/run.agent-adapter.test.ts
...
Use it from global init and uninstall; its absolute physical identity must agree with shell `cd -P`/`pwd -P` for existing directories. Do not use `CLOOKS_HOME_ROOT` for installation resolution. In `src/registration-state.ts` define:
