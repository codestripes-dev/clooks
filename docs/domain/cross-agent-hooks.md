...
|-------|------------|----------|-------------------|
| **Claude Code** | 21 events, 4 hook types | Mature | Yes (primary target) |
...
- **Not all events map.** Claude Code has 21 events; the others have 6-8. Events like `PreCompact`, `PostCompact`, `ConfigChange`, `TeammateIdle`, `TaskCreated`, `TaskCompleted`, `InstructionsLoaded`, `StopFailure` are Claude Code-specific. These would be available only when running under Claude Code.
