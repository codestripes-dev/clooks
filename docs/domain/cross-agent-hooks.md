...
Hook portability also depends on author tool assumptions: the existing `no-edit-protected` hook handles only Claude `Write`/`Edit`/`MultiEdit` with `filePath`, so it skips native Codex `apply_patch`; an owned fake-vendor probe confirmed an allowed patch on a vendor-pattern path, without modifying actual vendor files. See the [hook-specific compatibility finding](../findings/code-quality.md#protected-path-hook-assumes-claude-file-tools); event registration and matcher aliases do not make this hook portable.
...
The core `no-rm-rf` hook handles confirmation explicitly: an aggregate ask classification returns a deliberate block on Codex with a confirmation-unavailable explanation, while Claude retains ask. Classification, strict mode, allowlist and escape behavior are unchanged. This avoids relying on an unsupported result failing through the adapter; it does not fix the separate quoted-target parsing limitation or authorize a bypass.
