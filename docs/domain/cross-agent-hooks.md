...
| **Codex** | 12 documented events at September review; 11 targeted by Clooks | Active | Eleven-event runtime includes observational SessionEnd; historical ten-event Docker/native evidence remains separately scoped |
...
**Documentation snapshot, 2026-09-07:** The mapping table below preserves the review of [official hooks documentation](https://learn.chatgpt.com/docs/hooks). The [official changelog](https://learn.chatgpt.com/docs/changelog) announces Codex CLI `0.153.4` on 2026-09-04. The website also lists `SessionEnd` and `Interrupt`; the target at that review was ten events. The current implementation adds observation-only SessionEnd as its eleventh event; Interrupt remains excluded. Website claims are separate from the pinned-source corrections below, especially where PermissionRequest failure behavior contradicts them. Earlier expanded Docker validation covered ten events. Native proof is limited to the measured smoke capabilities below and the separately tested [orderly SessionEnd shutdown path](testing/codex-native.md#native-sessionend-shutdown).

- **Clooks target events:** `SessionStart`, `SubagentStart`, `PreToolUse`, `PermissionRequest`, `PostToolUse`, `PreCompact`, `PostCompact`, `UserPromptSubmit`, `SubagentStop`, `Stop`, `SessionEnd`. Matching Claude-side event names establish scope, not capability parity.
...
- **Timeout:** seconds, defaulting to 600 for most events. Native SessionEnd defaults to 1 second with a 3-second maximum; Clooks generates 3 seconds for its entire SessionEnd pipeline.
...
The following describes the current eleven-event implementation. PreToolUse and the earlier ten-event expansion/generated-error accounting correction have passed Docker validation; SessionEnd evidence is separately scoped below.
...
Claude remains the complete implemented adapter and retains its recursive key normalization, output translation, notify-only routing, ConfigChange downgrade and plugin/settings advisories. Codex's eleven-event implementation reuses the shared ordering/reduction boundary; registration emits one entrypoint per targeted event. SessionEnd alone registers `timeout: 3` seconds for the entire pipeline, not per hook; other events are unchanged. Init refresh adds it to old ten-event registrations and repairs owned SessionEnd entries without the timeout, retaining idempotent repeat and unrelated uninstall preservation. Native capability descriptions must distinguish emitted requests, inspected upstream consumers and actual execution evidence.
