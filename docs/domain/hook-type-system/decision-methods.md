...
Every context object carries per-event **decision methods** — `ctx.allow(...)`, `ctx.block(...)`, `ctx.skip(...)`, etc. — that construct the correct `*Result` shape without a hand-rolled object literal. This is the canonical idiom for returning from a handler.
...
The same public methods and runtime constructors remain attached for Codex. Constructors, including `ctx.ask({ reason })`, return synchronously; they neither prompt nor wait. The [shared engine checkpoint integration](../interactive-approvals.md#engine-checkpoints) performs the asynchronous wait after lifecycle and policy handling on both agents. Missing interaction refuses an ask rather than issuing retry tokens or emitting native Codex ask. Compiled validation does not establish generated-registration native conformance. Any string reason is valid, including blank strings with display-only fallback text. Sequential Codex ask patches use the same codec restrictions as allow; parallel rewrites, other-event handler asks and Codex defer remain refused. A dynamically returned `beforeHook` ask is unsupported: lifecycle warns, treats it as a no-op and continues the handler; the defensive policy rejects supplied before-hook-origin asks. The [historical token runtime](../codex-approvals.md) and its [15-case native suite](../testing/codex-native.md#hybrid-approval-case-evidence) do not establish new live-checkpoint behavior. Public method signatures remain unchanged.

Other unsupported fields are still refused rather than silently dropped: PermissionRequest permits only plain allow, block with reason, or skip/debug; no updated input/permissions or interrupt. PostToolUse has block/skip and supported context, not rewritten MCP output. UserPromptSubmit rejects sessionTitle. SessionStart/SubagentStart/PostCompact are observer handlers, so only skip is accepted (context only on the first two). A beforeHook block on an observer becomes an event-aware failure, not an ordinary observer result.
...
- `BEFORE_LIFECYCLE_METHODS = { block, skip, passthrough }` — same three universal runtime verbs regardless of event. The runtime `block` / `skip` constructors share one signature (typed as `BlockOptsRuntime` / `SkipOptsRuntime` in `src/engine/context-methods.ts`, which are deliberately wider than any per-event call site permits — `Reason & DebugMessage & Partial<InjectContext & Interrupt & UpdatedMcpToolOutput & SessionTitle>` for block, and `DebugMessage & Partial<InjectContext & UpdatedMcpToolOutput & SessionTitle>` for skip). The runtime spreads `{ ...opts }` and never inspects fields, so the loose typing reflects actual runtime behavior. The **type-level** opts each `BeforeHookEvent` variant accepts is narrowed per event via `LifecycleBlockOptsMap[K]` / `LifecycleSkipOptsMap[K]` on `BeforeHookEventVariants` — see "Per-event opts maps" below. `passthrough` is a small extra constructor returning a `LifecyclePassthroughResult` sentinel.
...
| `Reason` | `{ reason: string }` | Required reason. Shown to the agent (guard events) or user (continuation events). Pure single-field bag — `DebugMessage` is supplied via the bake. |
| `Feedback` | `{ feedback: string }` | Required next-turn instruction for a teammate (continuation events). Pure single-field bag. |
| `Path` | `{ path: string }` | Required absolute path (e.g. created worktree). Pure single-field bag. |
...
**`DebugMessage` baking convention.** `DebugMessage` is baked into both surfaces it touches:
...
**PostToolUse and PostToolUseFailure DU.** `PostToolUseContext` and `PostToolUseFailureContext` are discriminated unions over the 10 known tool names, mirroring `PreToolUseContext` / `PermissionRequestContext`. Authors writing a PostToolUse hook can narrow on `if (ctx.toolName === 'Bash')` and `ctx.toolInput` types as `BashToolInput`. The DU is constructed via the `ToolVariant<N, I>` helper plus a distributive `WithPostToolUseMethods<V>` type that intersects `PostToolUseDecisionMethods<Input>` per arm. Sibling types `UnknownPostToolUseContext` and `UnknownPostToolUseFailureContext` are exported for the unknown-tool case (MCP, future upstream tools), mirroring the established `UnknownPreToolUseContext` / `UnknownPermissionRequestContext` pattern.
...
**IDE tooltip note.** Some IDE configurations may render the named primitive form (e.g. `Skip<DebugMessage, R>`) at hover instead of the structurally-expanded opts shape; both are equivalent and assignment-compatible. The acceptance gate test in `test/types/decision-method-shape-coverage.types.ts` enforces the structural shape; what an IDE chooses to display on top of that is a presentation detail.

### Per-event opts maps

#### Surface split — lifecycle vs ctx

Two pairs of maps govern which opts each event accepts, split by surface:

**Wire-faithful ctx-side maps** (`EventBlockOptsMap` / `EventSkipOptsMap` in `src/types/method-primitives.ts`) — consumed by the `*DecisionMethods` types in `decision-methods.ts` and `contexts.ts`. These are the wire-faithful per-event opts: the full set of primitives Claude Code's wire format honors per event, including mutation primitives (`UpdatedMcpToolOutput` on `PostToolUse.block`, `SessionTitle` on `UserPromptSubmit.block/skip`). Mutations belong here because the per-event handler (`ctx.block(...)`, `ctx.skip(...)`) IS the correct surface for expressing output rewrites and session renames.

**Narrower lifecycle-side maps** (`LifecycleBlockOptsMap` / `LifecycleSkipOptsMap` in `src/types/method-primitives.ts`) — consumed by `BeforeHookEventVariants` in `lifecycle.ts`. These deliberately exclude mutation primitives (`UpdatedMcpToolOutput`, `SessionTitle`): lifecycle gates are metadata + communication only; mutations belong on per-event handlers, not on meta-gates. Entries for `PermissionRequest.block` retain `Interrupt` — that is a control-flow modifier on the block decision itself, not a content mutation. `InjectContext` is permitted on both surfaces for events whose wire format honors it (it is communication, not mutation).
...
Consumption: `BeforeHookEventVariants` (in `src/types/lifecycle.ts`) indexes `LifecycleBlockOptsMap[K]` / `LifecycleSkipOptsMap[K]` as `block(opts: LifecycleBlockOptsMap[K])` / `skip(opts?: LifecycleSkipOptsMap[K])` per variant. The `*DecisionMethods` types in `decision-methods.ts` / `contexts.ts` use `EventBlockOptsMap['<event>']` / `EventSkipOptsMap['<event>']` as the `O` parameter in their `Block<..., R>` / `Skip<..., R>` compositions.

Entries for events that have no `block` decision method at the ctx level (e.g., `WorktreeCreate`, `WorktreeRemove`, `PostCompact`, continuation events) exist solely to satisfy the `extends Record<EventName, unknown>` shape constraint. They are not consumed by any `*DecisionMethods` `Block<...>` arm, but are still required because `BeforeHookEventVariants` exposes `block` / `skip` universally on every event.

### Runtime-side composition

The runtime side of the decision-method system comprises 8 `*Opts` interfaces in `src/engine/context-methods.ts` (`AllowOpts`, `AskOpts`, `DeferOpts`, `SuccessOpts`, `FailureOpts`, `ContinueOpts`, `StopOpts`, `RetryOpts`) plus 2 local type aliases (`BlockOptsRuntime`, `SkipOptsRuntime`). The 8 interfaces compose from the same field-bag primitives via `extends`, inlining required fields (`reason`, `feedback`, `path`). The runtime constructors `block` and `skip` use the `*Runtime` aliases rather than per-event interfaces; uniform `BlockOpts` / `SkipOpts` interfaces are no longer used.

The two runtime type aliases (`BlockOptsRuntime`, `SkipOptsRuntime`) use intersection instead of `extends` and are deliberately wider than any per-event call site permits. This is required by function-parameter contravariance: the `block` / `skip` runtime constructors must be assignable to `BEFORE_LIFECYCLE_METHODS`, which in turn is assigned to per-variant lifecycle method signatures whose parameter types narrow to `LifecycleBlockOptsMap[K]` / `LifecycleSkipOptsMap[K]` (option (b) from the design). A structurally-permissive type alias (rather than a strict per-event interface) is the only choice that satisfies all 22 assignment sites simultaneously. The runtime spreads `{ ...opts }` and never inspects fields, so the loose typing reflects actual runtime behavior; the defensive field-dropping in `src/engine/translate.ts` stays as belt-and-suspenders for JS callers and engine-internal injection paths.

**Inline-vs-extends choice for required fields.** The required-field primitives `Reason`, `Feedback`, and `Path` are kept as inline `reason: string` / `feedback: string` / `path: string` declarations on the runtime opts interfaces rather than `extends Reason` etc. Those primitives are pure single-field bags (the universal `DebugMessage` is supplied via the type-source-side bake on method-shape primitives), so the runtime `*Opts` could `extends Reason` / `Feedback` / `Path` without inheritance noise. Inline was kept for stylistic continuity: the runtime opts surface is intentionally independent of the type-source bake, and switching the runtime composition pattern is orthogonal cleanup that adds risk for marginal benefit. Source-of-truth `*DecisionMethods` types in `decision-methods.ts` continue to compose from the field-bag primitives directly for one-line intersections; the asymmetry is intentional and ergonomic.
