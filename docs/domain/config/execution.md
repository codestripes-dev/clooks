...
- **Runtime** — Names in the order list must appear in the matched hook set for the current event. If an order list references a hook that does not handle the event (i.e., it was filtered out by `matchHooksForEvent()`), `orderHooksForEvent()` throws with a descriptive error. This check only runs when at least one hook matched: on an empty matched set the function returns immediately, and the engine's own no-match exit is reached first anyway. This is a structural/config error — the engine cannot proceed with an invalid order list. **Exception:** hooks skipped by the engine — `enabled: false`, or an `agents` list that does not contain the invoking agent — are silently skipped in the order list (not an error), so one `order:` list stays valid under every agent. A hook whose import failed is skipped the same way: its load error is reported on its own, and once its load-failure limit disables it the remaining ordered hooks still run. See `docs/domain/config/agents.md` for the allowlist cascade.
