...
- **`stale-registration`** — a plugin-vendored hook entry is identifiable through a Claude install manifest or a matching persisted install/settings plugin identity, but none of its known Claude keys is enabled at a scope supplying the same physical vendor destination. An enabled Codex pack at that destination also suppresses this warning, including equal home/project roots and directory aliases. Distinct user/project destinations remain separate; project/local activations share a vendor destination, so either covers local customization of a project-vendored hook. This does not change merge-origin handling for local overrides of home hooks without a project definition. Claude layers remain independent: local `false` does not cancel project `true`; managed activation is excluded. Unknown-origin copies produce no Claude stale-registration claim. A removed cache may remain identifiable from its retained native key; removing all identifying records deliberately loses that diagnostic.
