...
The repository's core `no-rm-rf` hook returns `ctx.ask` for confirmation classifications on both Claude and Codex. Claude uses native confirmation; Codex requires a Clooks runtime with the [hybrid approval fallback](../codex-approvals.md). Pending operations are denied until their exact confirmations are acknowledged. Project-root and non-allowlisted project targets still become explicit blocks under strict mode, and the existing `ALLOW_DESTRUCTIVE_RM=true` escape does not discharge asks or strict-mode blocks. Deny precedence, classification, allowlist and other escape rules are unchanged. This local branch removal does not update marketplace or global pack copies. The passing [native suite](../testing/codex-native.md#hybrid-approval-case-evidence) verifies actual-pack `rm -r` with two confirmations, target effects and replay refusal, not native `rm -rf` permission or full conformance.
