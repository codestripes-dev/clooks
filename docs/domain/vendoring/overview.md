...
The vendored project pack's `no-edit-protected` lock defaults include `bun.lock` and `**/` patterns for root and nested workspace lockfiles, retaining the existing rule-group toggle. Native patch-path inspection uses the existing rules as described below. Core agent differences are described in [Cross-Agent Hook Systems](../cross-agent-hooks.md).

### Hook Inspection Boundaries

The repository's core `no-rm-rf` hook returns synchronous `ctx.ask` results for confirmation classifications on both Claude and Codex. The engine resolves these through [live checkpoints](../interactive-approvals/engine-checkpoints.md#engine-checkpoints), replacing the [historical token fallback](../codex-approvals.md). Project-root and non-allowlisted project targets still become explicit blocks under strict mode, and `ALLOW_DESTRUCTIVE_RM=true` does not discharge asks or strict-mode blocks. Deny precedence, classification, allowlist and other escape rules are unchanged. This local change does not update marketplace or global pack copies. The historical [native suite](../testing/codex-native/approval-cases.md#hybrid-approval-case-evidence) verifies actual-pack `rm -r` with token confirmations, target effects and replay refusal, not new live-checkpoint behavior, native `rm -rf` permission or full conformance.

Quoted-target parsing remains limited: sanitization can discard single-quoted targets, while double-quoted paths containing spaces can lose argument boundaries. Hybrid approval does not fix that parser or prove that classification matches every shell target.
...
The core pack's `tmux-notifications` colors window status on Stop when attentionOnStop is enabled and resets attention on focus or supported activity. Explicit Codex PermissionRequest reuses attention styling and optional focused-window flash, then skips without deciding approval; Claude and absent-agent contexts skip this handler without duplicate notifications. This native approval-request signal may auto-resolve: it is not proof of a displayed prompt, a generic user-question signal, or the hybrid ctx.ask PreToolUse denial. Existing SessionEnd cleanup restores window styles and automatic rename; Codex needs eleven-event registration refreshed by init. No idle or Interrupt event is synthesized. It uses shared server state: the configured indexed hook slot must be free, because installing there can replace its binding while preserving other indices. A sentinel suppresses repeat installation without migrating a changed slot. Claude notification feedback does not imply corresponding Codex events. Existing shell interpolation and shared-style limitations remain. See the [core pack README](../../../.clooks/vendor/plugin/clooks-core-hooks/README.md) for settings and the free-slot requirement.
