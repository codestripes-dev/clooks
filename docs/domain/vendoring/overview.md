...
The repository's core `no-rm-rf` hook returns synchronous `ctx.ask` results for confirmation classifications on both Claude and Codex. The engine resolves these through [live checkpoints](../interactive-approvals.md#engine-checkpoints), replacing the [historical token fallback](../codex-approvals.md). Project-root and non-allowlisted project targets still become explicit blocks under strict mode, and `ALLOW_DESTRUCTIVE_RM=true` does not discharge asks or strict-mode blocks. Deny precedence, classification, allowlist and other escape rules are unchanged. This local change does not update marketplace or global pack copies. The historical [native suite](../testing/codex-native.md#hybrid-approval-case-evidence) verifies actual-pack `rm -r` with token confirmations, target effects and replay refusal, not new live-checkpoint behavior, native `rm -rf` permission or full conformance.
