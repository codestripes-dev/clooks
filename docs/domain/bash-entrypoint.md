...
## Launcher Metadata and Refresh Guard

Generated project and global launchers include these exact data lines after their existing managed scope header:
...
The values come from `src/installation-metadata.ts`; the template imports that lightweight module without loading the filesystem/registration inspector on ordinary runtime startup. The one launcher revision covers the coupled launcher/advisory integration behavior, and tests pin both complete generated artifacts for that revision; there is no separate advisory revision. The minimum runtime is an independent SemVer floor. Neither field proves that hook, MCP or advisory registration completed.

`clooks init --check` reads the launcher as bounded regular-file data and never sources it. Metadata is valid only as exactly one nonnegative decimal revision comment plus one exact single-quoted canonical SemVer assignment. Both absent is a legacy managed launcher; partial, duplicate, indented or malformed metadata is uninspectable. A higher revision is future and is never offered an init downgrade. A satisfied future floor stays quiet; an unmet floor remains a binary-update need without an integration repair.

The generated `runtime-advisory.sh` carries its own exact managed scope header and a minimum-runtime literal from the same constant. Inspection takes the greater valid launcher/advisory floor and treats mismatched literals, a missing advisory file or missing per-agent advisory registration as an integration refresh.

Before mutating init writes, the same parser rejects malformed metadata, wrong managed scope, future launcher revisions and minimum floors in either generated file newer than the selected runtime. Missing or headerless custom launchers retain historical direct-init behavior, but a headerless custom advisory is never overwritten. This guard cannot protect generated files from init binaries released before the guard existed.

## Runtime Update Advisory

`clooks init` writes `.clooks/bin/runtime-advisory.sh` beside the runtime launcher and registers it as a separate SessionStart matcher group for each selected agent. It is never added to PreToolUse, paired approval handling or the runtime matcher group. Project and global advisories remain independent; global runtime dedup does not hide a project floor.

The advisory drains stdin, exits quietly only for exact `SKIP_CLOOKS=true`, and performs no dedup or approval suppression. It resolves `clooks` from the inherited PATH, runs `--version` with detached stdin, accepts only strict SemVer output and emits fixed, valid agent JSON only when the selected binary is below the generated floor. Missing, failing or malformed probes are silent exit 0. It does not write state or update anything.

Registration commands set `CLOOKS_AGENT` explicitly. Claude project/global and Codex global commands use exact guarded `sh -c` wrappers; Codex project registration uses a separately pinned ancestor locator labelled `clooks-advisory-project`. Every wrapper checks that the generated file exists and is readable before `exec bash`, so a stale native registration left by an older full uninstaller stays silent. Advisory ownership is strict and separate from launcher ownership; runtime command predicates are unchanged.
