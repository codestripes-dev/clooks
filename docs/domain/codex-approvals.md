...
[shared live approval checkpoints](interactive-approvals.md#engine-checkpoints)
without replaying earlier hooks. The public `ctx.ask` API is unchanged;
`ctx.defer` remains a separate result with its existing provider restrictions.
...
The [historical native hybrid receipts](testing/codex-native.md#hybrid-approval-case-evidence)
