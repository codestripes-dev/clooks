...
An invocation-specific result policy reaches the executor as its optional tenth argument. The executor audits author-capability and parallel-contract results immediately before their effects. Codex's `deferRuntimeErrorAudit` instead lets ordinary error capture, onError selection and counter accounting precede audits of selected blocking errors. Any latched `policyFailure` returns separately from reduced results. The engine selects `translateFailure` when that failure exists, otherwise `translateFinalOutput`; even an allow returned by diagnostic composition or final adjustment cannot select normal translation after rejection. Failure translation receives the private invocation, the failure and the warnings accumulated so far, and its exit code is authoritative: an adapter may express a refusal as exit-0 JSON. No private envelope is spread into hook contexts, result JSON or diagnostic text. This is an internal extension; there is no new public agent/raw field or decision method.

Accumulated warnings — shadowing, dangling hook files, `order:` entries naming a disabled hook, unknown agent ids, load errors and their repair text, hooks disabled after repeated failures — reach failure output through `systemMessages` on the failure-translation input, in the same order the success path uses: plugin, dangling, startup, execution. The engine keeps them on its invocation state so the exits outside the invocation body (a caught runtime failure, a failed approval close, a cancellation) can deliver them too. One helper publishes the current list, and it runs whenever a list becomes final: as soon as plugin preparation returns and before that step's cancellation check, after the dangling scan, after the `SessionStart` advisories, after the startup warnings are assembled, at each of the two early exits with the list that exit itself emits, before hook execution starts, immediately after execution returns, again once diagnostic composition has added its own and before the post-execution cancellation check, and once more after the final adjustment. Composition runs before that cancellation check on purpose: it is a pure translation, and on Codex it is what turns a degradation notice into a user-facing message, so running it later would lose that notice whenever a hook cancelled the invocation. Only what a composer returns as system messages is published; its stderr and context lines stay on their own channels, and its stderr is still written after the check. The one degraded-config exit that predates these lists writes its single warning to the state directly.

What is guaranteed is therefore per list, not per run. Any failure that reaches failure translation carries every list that was complete when it happened. A step that throws while assembling its own messages loses those messages, and that includes an adapter callback and the engine's own startup loops. The direct stderr exits for malformed late stdin translate nothing, so they deliver nothing either, unless a close failure or a cancellation afterwards replaces the output. `executeHooks` is the widest such step: it owns its arrays and returns them only on a normal return, so anything it collected before throwing is unreachable.

The engine never writes warnings to stderr itself, and the decision, reason, stop-reason and exit-code fields of a failure are identical with and without them. Each adapter routes them: Codex's exit-0 JSON failures append them to `systemMessage` after the failure text, which that field already carries; Claude's one JSON failure (`PreToolUse`) puts the warnings alone in `systemMessage`, because its failure text belongs in `hookSpecificOutput.permissionDecisionReason`. Claude's exit-2 failures append to stderr after a blank line on `Notification`, `SubagentStart`, `SessionStart` and `SessionEnd` — the four routes whose exit-2 stderr upstream shows to the user and to nobody else — and leave every other route alone. That omission is a deliberate policy, not an upstream constraint. The other routes differ from each other: some feed the same stderr to the model, where a warning would read as part of the block reason; others ignore it, or keep it for debug output only. None of them is a channel a warning can be added to without changing what the agent or the model is told, so none of them gets one. Codex's stderr-only failures (`SubagentStart`, `PostCompact`, `SessionEnd`, and unhandled or unidentified events) append after a blank line as best effort, since Codex does not guarantee that stderr reaches the user. Typed approval refusals omit them on both agents — see [Engine Checkpoints](../interactive-approvals/engine-checkpoints.md). Failures raised before any warning exists, such as stdin and config validation, are unchanged.
