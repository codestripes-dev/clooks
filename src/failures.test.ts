...
describe("recordFailure", () => {
  test("creates a new entry for a previously unseen hook+event", () => {
    const state = recordFailure({}, "my-hook", "PreToolUse", "boom")
    expect(state["my-hook"]!["PreToolUse"]!.consecutiveFailures).toBe(1)
    expect(state["my-hook"]!["PreToolUse"]!.lastError).toBe("boom")
    expect(state["my-hook"]!["PreToolUse"]!.lastFailedAt).toBeTruthy()
  })

  test("increments an existing entry", () => {
    const initial = recordFailure({}, "my-hook", "PreToolUse", "first error")
    const updated = recordFailure(initial, "my-hook", "PreToolUse", "second error")
    expect(updated["my-hook"]!["PreToolUse"]!.consecutiveFailures).toBe(2)
    expect(updated["my-hook"]!["PreToolUse"]!.lastError).toBe("second error")
  })
})

describe("clearFailure", () => {
  test("removes a hook+event entry", () => {
    const initial = recordFailure({}, "my-hook", "PreToolUse", "boom")
    const cleared = clearFailure(initial, "my-hook", "PreToolUse")
    expect(cleared["my-hook"]).toBeUndefined()
  })

  test("removes the hook-level key when its last event entry is cleared", () => {
    let state = recordFailure({}, "my-hook", "PreToolUse", "boom")
    state = recordFailure(state, "my-hook", "PostToolUse", "boom2")
    const cleared = clearFailure(state, "my-hook", "PreToolUse")
    expect(cleared["my-hook"]).toBeDefined()
    expect(cleared["my-hook"]!["PreToolUse"]).toBeUndefined()
    expect(cleared["my-hook"]!["PostToolUse"]).toBeDefined()

    const cleared2 = clearFailure(cleared, "my-hook", "PostToolUse")
    expect(cleared2["my-hook"]).toBeUndefined()
  })

  test("is a no-op when the entry does not exist", () => {
    const state = recordFailure({}, "my-hook", "PreToolUse", "boom")
    const result = clearFailure(state, "other-hook", "PreToolUse")
...
describe("getFailureCount", () => {
  test("returns 0 for unknown hook+event", () => {
    expect(getFailureCount({}, "unknown", "PreToolUse")).toBe(0)
  })

  test("returns the count for a known entry", () => {
    let state = recordFailure({}, "my-hook", "PreToolUse", "boom")
    state = recordFailure(state, "my-hook", "PreToolUse", "boom2")
    expect(getFailureCount(state, "my-hook", "PreToolUse")).toBe(2)
  })
})
