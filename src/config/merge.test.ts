...
describe("mergeThreeLayerConfig", () => {
  ...
  test("local cannot define new hooks → throws", () => {
    const project = {
      version: "1.0.0",
      "project-hook": {},
    }
    ...
    expect(() => mergeThreeLayerConfig(undefined, project, local)).toThrow(
      'local config defines hook "new-hook" which is not defined in home or project config',
    )
  })
  ...
  test("local config defines new hook not in home or project → error", () => {
    const project = {
      version: "1.0.0",
      "project-hook": {},
    }
  ...
  test("homeHookUses tracks original home hook uses before local override", () => {
    ...
    expect((result.merged["home-hook"] as Record<string, unknown>).uses).toBe("./overridden/path.ts")
  })
