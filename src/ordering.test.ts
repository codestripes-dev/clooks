...
describe('orderHooksForEvent', () => {
  test('empty matched set returns empty array', () => {
    const result = orderHooksForEvent(
      [],
      undefined,
  ...
  test('no order list, mixed parallel/sequential: parallel hoisted to front', () => {
    const matched = [makeLoaded('seqA'), makeLoaded('parB'), makeLoaded('seqC'), makeLoaded('parD')]
    const hookEntries = makeEntries({
  ...
  test('no order list with empty order array: treated as no order list', () => {
    const matched = [makeLoaded('seqA'), makeLoaded('parB'), makeLoaded('seqC')]
    const hookEntries = makeEntries({
      seqA: makeEntry(false),
      parB: makeEntry(true),
      seqC: makeEntry(false),
  ...
  test('order list references hook NOT in disabledNames and NOT in matched set: still throws', () => {
    ...
    expect(() =>
      orderHooksForEvent(matched, eventEntry, hookEntries, 'PreToolUse', disabledNames),
    ).toThrow('event "PreToolUse" order references hook "hookB" which does not handle this event')
  })
...
describe('partitionIntoGroups', () => {
  ...
  test('all sequential: one group', () => {
    const hooks = [makeOrdered('a', false), makeOrdered('b', false), makeOrdered('c', false)]
  ...
  test('all parallel: one group', () => {
    const hooks = [makeOrdered('a', true), makeOrdered('b', true), makeOrdered('c', true)]
  ...
  test('single parallel hook between sequential groups: warns to stderr', () => {
    ...
    const hooks = [makeOrdered('a', false), makeOrdered('lonely', true), makeOrdered('b', false)]
  ...
  test('single parallel hook between parallel and sequential: no warning', () => {
    ...
    const hooks = [makeOrdered('a', true), makeOrdered('b', true), makeOrdered('c', false)]
