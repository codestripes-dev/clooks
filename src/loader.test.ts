...
function makeHookEntry(resolvedPath: string, config: Record<string, unknown> = {}): HookEntry {
  return {
    resolvedPath,
    config,
    parallel: false,
...
function makeConfig(hooks: Record<string, HookEntry>): ClooksConfig {
...
describe('validateHookExport', () => {
  ...
  test('throws when a non-meta property is not a function', () => {
    ...
    expect(() => validateHookExport(mod, 'test.ts')).toThrow('hook.PreToolUse is not a function')
  })
  ...
  test('rejects unknown property with helpful error', () => {
    expect(() =>
      validateHookExport({ hook: { meta: { name: 'x' }, beforHook: () => {} } }, 'test.ts'),
...
describe('loadHook', () => {
  ...
  test('throws when hook file does not exist', async () => {
    ...
    expect(loadHook(hn('missing'), entry, dir)).rejects.toThrow('failed to import hook "missing"')
  })
...
describe('loadHook with aliases', () => {
  ...
  test('short address uses loads when meta.name matches hook-name portion', async () => {
    const dir = makeTempDir()
    ...
    const entry = makeAliasEntry(hookFile, 'someuser/hooks:lint-guard')
    const result = await loadHook(hn('lint-guard'), entry, dir)
    expect(result.name).toBe(hn('lint-guard'))
    expect(result.hook.meta.name).toBe(hn('lint-guard'))
  })

  test('short address uses throws when meta.name mismatches hook-name portion', async () => {
    const dir = makeTempDir()
  ...
  test('short address uses throws even when meta.name matches YAML key but not hook-name', async () => {
    const dir = makeTempDir()
    const hookFile = join(dir, 'alias.ts')
    writeFileSync(
      hookFile,
      `export const hook = {
        meta: { name: "my-alias" },
